...
/* TIME COMPLEXITY
Let the time complexity be denoted as T(n).
T(n)=2*T(n/2) + O(n) + O(n^2) + O(n)
This is because, the sorting algorithm used in this code uses 0(n^2). and the points are divided into two sets, so 2*T(N/2), and 2*O(n), because it finds the strips and the closest points in O(n) time. Better sorting techniques like, merge sort can be used to reduce the complexity too.
